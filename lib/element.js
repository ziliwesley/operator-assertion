/**
 * Element module
 * See: [element](http://docs.mongodb.org/manual/reference/operator/query/#element)
 */
...
extTypes.Date = {
    validate: function (a, pattern) {
        switch (pattern) {
            case 'ISO':
                return new Date(a).toISOString() === a;
            case 'GMT':
                return new Date(a).toGMTString() === a;
            case 'UTC':
                return new Date(a).toUTCString() === a;
            default: 
...
extTypes.Timestamp = {
    validate: function (a, pattern) {
        return new Date().toFormat(pattern) === a;
    }
};
